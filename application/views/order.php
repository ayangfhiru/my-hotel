...
<div class="px-10 space-y-3 py-10">
    <div class="flex flex-col space-y-20 2xl:flex-row 2xl:space-x-11">
        <div class="w-full 2xl:flex-1">
            ...
            <div class="w-full overflow-x-scroll no-scrollbar">
                <table class="w-full">
                    <tbody>
                        <?php foreach ($bookings as $index => $bo): ?>
                            <?php
                            $statusClass = match ($bo->payment_status) {
                                'pending' => 'text-bamber-500',
                                'completed' => 'text-success-300',
                                'failed' => 'text-red-500',
                                default => '',
                            };
                            ?>
                            <tr class="mb-5 <?= $index % 2 == 0 ? 'bg-white dark:bg-darkblack-600' : 'bg-white dark:bg-darkblack-400' ?>">
                                <td
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div>
                                                <span class="text-base font-medium text-bgray-700 dark:text-bgray-50">
                                                    <?= date('d M Y', strtotime($bo->booking_time)); ?>
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <td class="whitespace-nowrap px-2 py-4 text-sm text-gray-500">
                                    <span class="text-am rounded-lg px-3 py-1 text-base font-medium">
                                        Rp <?= number_format($bo->amount); ?>
                                    </span>
                                </td>
                                <td
                                    class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    <span id="timer-<?= $bo->payment_id ?>" class="countdown-timer rounded-lg bg-success-50 px-3 py-1 text-sm font-medium text-success-300 dark:bg-darkblack-500" data-expire-time="<?= $bo->payment_deadline ?>"></span>
                                </td>
                                <td class="whitespace-nowrap px-2 py-4 text-sm text-gray-500">
                                    <span class="text-am rounded-lg px-3 py-1 text-sm font-medium <?= $statusClass ?>">
                                        <?= $bo->payment_status; ?>
                                    </span>
                                </td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    <div class="flex justify-end">
                                        <a href="<?= site_url("payments/$bo->snap_token/pay-now") ?>" class="ml-6 rounded-xl bg-success-300 px-11 py-3 font-semibold text-white transition duration-300 ease-in-out hover:bg-success-400" target="_blank">
                                            Pay
                                        </a>
                                    </div>
                                </td>
