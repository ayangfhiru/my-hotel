...
<section class="bg-white py-3 antialiased md:pt-16">
    <div class="mx-auto px-4 2xl:px-0">
        <h2 class="text-xl font-semibold text-gray-900 sm:text-2xl">Shopping Cart</h2>
        <?= form_open("booking/multiple"); ?>
        <div class="mt-6 sm:mt-8 md:gap-6 lg:flex lg:items-start xl:gap-8">
            <div class="mx-auto w-full flex-none lg:max-w-2xl xl:max-w-4xl">
                <div class="space-y-6 flex flex-col">
                    <?php foreach ($carts as $index => $cart): ?>
                        <?php $isDisabled = $cart->check_in < date('Y-m-d'); ?>
                        <!-- Hidden inputs for dates and price -->
                        <input type="hidden" name="check_in[<?= $index ?>]" value="<?= date('d M Y', strtotime($cart->check_in)) ?>">
                        <input type="hidden" name="check_out[<?= $index ?>]" value="<?= date('d M Y', strtotime($cart->check_out)) ?>">
                        <input type="hidden" name="price[<?= $index ?>]" value="<?= $cart->price ?>">
                        <input type="hidden" name="room_type[<?= $index ?>]" value="<?= $cart->room_type ?>">

                        <label id="list-cart-<?= $cart->room_id ?>" for="cart-<?= $index ?>">
                            <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6">
                                <div class="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                                    <input type="checkbox"
                                        name="selected_room[<?= $index ?>]"
                                        id="cart-<?= $index ?>"
                                        value="<?= $cart->room_id ?>"
                                        <?= $isDisabled ? 'disabled' : '' ?>>
                                    <div class="shrink-0 md:order-1">
                                        <img class="h-20 w-20 dark:hidden object-cover"
                                            src="<?= site_url("assets/images/thumbnail/" . ($cart->thumbnail ?? 'default.jpg')) ?>"
                                            alt="<?= $cart->room_type ?>" />
                                        <img class="hidden h-20 w-20 dark:block object-cover"
                                            src="<?= site_url("assets/images/thumbnail/" . ($cart->thumbnail ?? 'default.jpg')) ?>"
                                            alt="<?= $cart->room_type ?>" />
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between md:order-3 md:justify-end">
                                        <div class="flex items-center">
                                            ...
                                            <?= form_input("quantity[$index]", $cart->quantity, [
                                                'type'  => 'number',
                                                'id'    => 'counter-input',
                                        ...
                                        <div class="text-end md:order-4 md:w-32">
                                            <p class="text-base font-bold text-gray-900 dark:text-white">Rp <?= number_format($cart->price); ?></p>
                                        </div>
                                    </div>
                                    <div class="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
                                        ...
                                        <table class="text-base font-medium border-spacing-2 text-gray-900 dark:text-white">
                                            <tr>
                                                <td>Check In</td>
                                                <td>
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            <button type="button"
                                                ...
                                                class="removeCart inline-flex items-center text-sm font-medium text-red-600 hover:underline dark:text-red-500">
                                                <svg class="me-1.5 h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
